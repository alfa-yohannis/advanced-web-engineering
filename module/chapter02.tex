...
driver kartu grafis hanya menjatuhkan proses GPU. Browser tinggal menjalankan
...
menaikkan pemakaian memori secara nyata, karena tiap proses membawa salinan
...
di \textit{edge server} terdekat, bukan langsung ke \textit{origin}. Bila salinan
...
melayani permintaan berikutnya dari salinan tersebut.
...
\begin{lstlisting}[language=html, caption={Membuka koneksi ke domain pihak ketiga lebih awal, tersedia di \berkas{module/sources/chapter02/preconnect.html}}, label={lst:preconnect}]
...
\begin{lstlisting}[language=bash, caption={Mengukur waktu tiap tahap koneksi, format keluarannya di \berkas{module/sources/chapter02/format-curl.txt}}, label={lst:curl}]
...
\berkas{module/sources/chapter02/}. Skrip \texttt{ukur-koneksi.sh} menjalankan perintah
...
Header juga dimampatkan memakai HPACK \cite{rfc7541}. Nama HPACK bukan
...
Header dimampatkan memakai QPACK \cite{rfc9204}, yaitu penyesuaian HPACK untuk
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{lXXX}
    ...
    \hline
    Header & Dikirim penuh & Dimampatkan HPACK & Dimampatkan QPACK \\
    \hline
...
tiap putaran. Browser hanya menggambar mengikuti laju penyegaran layar, umumnya
...
\begin{lstlisting}[language=TypeScript, caption={Urutan eksekusi tugas dan \textit{microtask}, tersedia di \berkas{module/sources/chapter02/urutan-event-loop.js}}, label={lst:urutan}]
...
\begin{lstlisting}[language=TypeScript, caption={Tugas berat yang menahan seluruh antarmuka, tersedia di \berkas{module/sources/chapter02/tugas-berat.ts}}, label={lst:longtask}]
...
\begin{lstlisting}[language=html, caption={Script yang menahan \textit{parsing} dan perbaikannya, tersedia di \berkas{module/sources/chapter02/defer-async.html}}, label={lst:defer}]
...
Pengukuran wajib dilakukan dengan pembatasan yang mendekati kondisi pengguna.
Aktifkan \textit{network throttling} pada profil Slow 4G. Aktifkan juga pembatasan kecepatan Central Processing Unit (CPU) pada faktor
4 kali lebih lambat. Tanpa pembatasan itu,
...
\berkas{module/sources/chapter02/}, dihitung relatif terhadap direktori
\texttt{materi}.
...
\begin{enumerate}
  ...
  \item Buka DevTools, aktifkan Slow 4G dan pembatasan CPU 4 kali lebih lambat,
  ...
  \item Ukur TTFB memakai \berkas{module/sources/chapter02/ukur-koneksi.sh}
  ...
  \item Ulangi langkah 2 sampai 6 pada satu halaman dalam, yaitu
        ...
        yaitu profil jaringan, faktor pembatasan CPU, dan mode tampilan, agar
...
\begin{lstlisting}[language=bash, caption={Uji beban dengan k6, tersedia di \berkas{module/sources/chapter02/skrip-beban.js}, hanya untuk sistem sendiri atau yang sudah berizin}, label={lst:k6}]
...
\begin{lstlisting}[language=bash, caption={Skrip pengukur tiap tahap pembukaan koneksi, tersedia di \berkas{module/sources/chapter02/ukur-koneksi.sh}}, label={lst:lat1}]
#!/usr/bin/env bash
...
\berkas{module/sources/chapter02/ukur-koneksi.sh}, dihitung relatif terhadap
direktori \texttt{materi}. Jalankan dengan perintah berikut.

\begin{lstlisting}[language=bash, caption={Menjalankan skrip pengukur koneksi}, label={lst:lat1-jalan}]
cd module/sources/chapter02
...
yang terakhir diubah lebih dari setahun lalu dianggap masih segar untuk waktu
...
\begin{table}[ht]
  \centering
  \caption{Contoh hasil Latihan 2 yang sudah terisi, lima aset milik
           \texttt{www.pradita.ac.id}}
...
\textit{multiplexing}. Hasil akhirnya mirip, tetapi ongkosnya berbeda.
