...
\begin{frame}{\hfill}
  \centering
  \Huge{\textbf{Kapan \textit{copy} boleh dipakai,\\dan berapa lama menunggu?}}
\end{frame}
...
\begin{frame}{{\Large Tujuan Pembelajaran}}
  ...
  \begin{itemize}
    ...
    \item Mengukur untung rugi pola \textit{cache-aside}, termasuk data \textit{stale}
          dan lonjakan \textit{query} yang muncul ketika sebuah kunci kedaluwarsa.
...
\begin{frame}{{\Large Dua Pertanyaan Bab Ini}}
  \vspace{10pt}
  \begin{itemize}
    \item \textit{Query} buku terlaris 30 hari pada Bab 3 tetap memakan puluhan
    ...
    \item \textit{Caching} menyimpan jawaban yang sudah jadi, lalu memakainya
          lagi. \textit{Tradeoff}-nya: \textit{copy} dapat \textit{stale}, dan tempat
          penyimpanannya dapat \textit{slow} atau \textit{down}.
  \end{itemize}
  \vspace{8pt}
  {\footnotesize Dua pertanyaan: kapan sebuah \textit{copy} boleh dipakai, dan apa
...
\begin{frame}{{\Large Fresh atau Stale}}
  \vspace{10pt}
  \begin{itemize}
    \item \textit{Copy} yang masih \textit{fresh} langsung dipakai, tanpa menghubungi server
          sama sekali.
    \item \textit{Copy} yang \textit{stale} ditanyakan ulang. Browser mengirim tanda versi
          \textit{copy}-nya lewat \texttt{If-None-Match}, berisi nilai \texttt{ETag}
          dari server.
  ...
  \end{itemize}
  \vspace{6pt}
  {\footnotesize \texttt{ETag} adalah tanda versi isi jawaban, dibuat server
  dari isi berkasnya. Begitu isinya berubah, nilainya ikut berubah.

  \vspace{4pt}
  Analoginya fotokopi jadwal ujian bertuliskan ``berlaku sampai Jumat'' dan
  ``versi 7''. Setelah Jumat, mahasiswa cukup bertanya ``versi 7 masih
  berlaku?'', dan petugas cukup menjawab ``masih''.}
\end{frame}
...
\begin{frame}{{\Large Bila Server Tidak Menulis Kebijakan}}
  ...
  \begin{itemize}
    ...
    \item Request for Comments (RFC) 9111 membolehkan browser menebak masa
          \textit{fresh} dari umur berkas. Angka yang lazim adalah sepersepuluh selang
          sejak berkas terakhir diubah.
    \item Berkas diubah 14 Maret 2025. Pada 10 September 2026 umurnya sekitar
          545 hari, sehingga dianggap \textit{fresh} sekitar 54 hari.
  \end{itemize}
  \vspace{8pt}
  {\footnotesize Itulah sebab aset yang tercatat 0 milidetik pada Latihan 3 di
  Bab 2.}
\end{frame}
...
\begin{frame}{{\Large CDN Adalah Cache Bersama}}
  ...
  \begin{enumerate}
    ...
    \item Halaman yang memuat nama pengguna keliru diberi \texttt{public}.
          \textit{Copy} milik pengguna pertama terkirim ke pengguna berikutnya.
          Halaman semacam itu diberi \texttt{private}.
  \end{enumerate}
  \vspace{6pt}
  {\footnotesize \texttt{Vary: Accept-Encoding} wajar. \texttt{Vary: Cookie}
  membuat tiap pengguna punya \textit{copy} sendiri, sehingga CDN hampir tidak pernah
  menemukan \textit{copy} yang dapat dipakai ulang.}
\end{frame}

\begin{frame}{{\Large Kapan Hasil Query Layak Disimpan}}
  \vspace{10pt}
  \begin{itemize}
    \item Tiga syaratnya: beban komputasi mahal, lebih sering dibaca daripada
          diubah, dan boleh sedikit \textit{stale}.
...
\begin{frame}{{\Large Pola Cache-Aside}}
  \vspace{4pt}
  {\footnotesize \textit{Cache-aside} berarti aplikasi sendiri yang mengurus
  \textit{cache}-nya: aplikasi memeriksa \textit{cache} lebih dahulu, dan
  mengisinya sendiri ketika kosong.}
  \vspace{4pt}
  \begin{itemize}
    ...
    \item Redis tidak pernah menjadi \textit{source of truth}. Bila Redis kosong atau
          \textit{down}, jawaban tetap benar, hanya lebih lambat.
  \end{itemize}
  \vspace{4pt}
  {\footnotesize HIT dan MISS bukan singkatan, melainkan kata Inggris untuk
  ketemu dan tidak ketemu, dipakai sebagai nama status \textit{cache}.

  \vspace{4pt}
  Analoginya warung dengan kulkas di depan. Bila kulkas kosong, penjual
  mengambil dari gudang belakang sekalian mengisi kulkas. Kulkas boleh kosong
  kapan saja, dan warung tetap berjualan.}
\end{frame}
...
\begin{frame}{{\Large Cache yang Macet}}
  ...
  \begin{itemize}
    ...
    \item Tiap permintaan menunggu 50 milidetik untuk GET, menjalankan \textit{query},
...
\begin{frame}{{\Large Dua Konsekuensi dan Satu Peringatan}}
  \vspace{8pt}
  \begin{enumerate}
    \item Kegagalan \textit{cache} tidak boleh meng-\textit{crash} aplikasi. Setiap
    ...
    \item Setiap panggilan ke \textit{cache} membutuhkan batas waktu yang jauh
          lebih kecil daripada \textit{query} yang ingin dihindarinya.
  \end{enumerate}
  \vspace{6pt}
...
\begin{frame}{{\Large TTL dan Invalidasi}}
  \vspace{10pt}
  \begin{itemize}
    \item \textit{Copy} menjadi \textit{stale} ketika data aslinya berubah.
    \item TTL: \textit{copy} kedaluwarsa sendiri setelah sekian detik.
    \item Invalidasi: kode yang mengubah data langsung menghapus \textit{copy}-nya,
...
\begin{frame}{{\Large Cara Membatasi Data Stale}}
  \vspace{4pt}
  \centering
  {\scriptsize
  \begin{tabularx}{\textwidth}{lYY}
...
\end{lstlisting}
  ...
  \vspace{4pt}
  {\footnotesize Jendela \textit{stale} menyusut menjadi 0,5 detik. Cara yang lebih rapi
...
\begin{frame}{{\Large Berapa Panjang TTL}}
  ...
  \begin{enumerate}
    ...
    \item Harga dan stok pada saat pembayaran tidak dibaca dari
          \textit{cache} sama sekali. Harga yang \textit{stale} 5 detik saja dapat
...
\begin{frame}{{\Large Cache Stampede}}
  ...
  \begin{itemize}
    \item Kunci yang sering dibaca kedaluwarsa, lalu banyak permintaan meleset
          pada saat yang sama dan menjalankan \textit{query} yang sama.
...
\begin{frame}{{\Large Hasil: 50 Permintaan Serentak}}
  ...
  \vspace{6pt}
  \begin{itemize}
    \item Tanpa pengaman, 50 \textit{query} mengantre dalam lima gelombang: p50-nya
          hampir tujuh kali lipat satu \textit{query} yang berjalan sendirian, yaitu
          135,2 milidetik.
    \item Dengan \textit{single-flight}, hanya satu \textit{query} yang sampai ke basis
          data.
  \end{itemize}
\end{frame}
...
\begin{frame}{{\Large Dua Batas Waktu}}
  ...
  \texttt{Cache-Control: max-age=60, stale-while-revalidate=240}. \textit{Copy} tetap \textit{fresh}
...
\begin{frame}{{\Large Satu Angka yang Janggal}}
  ...
  \end{itemize}
  \vspace{8pt}
  {\footnotesize Cara lain, yaitu me-\textit{refresh} lebih awal secara acak sebelum
...
\begin{frame}{{\Large Eventual Consistency}}
  \vspace{10pt}
  \begin{itemize}
    \item \textit{Copy} di browser, CDN, dan Redis tidak berubah pada saat yang sama
          ketika data diubah.
    \item Bila tidak ada perubahan baru, semua \textit{copy} pada akhirnya kembali
          sama.
...
\end{lstlisting}
  ...
  komputer sendiri. Permintaan ke sistem luar hanya tiga, ke satu aset situs
  kampus, dan ketiganya bersifat mengamati.}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \texttt{stampede.py}, \texttt{resilience.py}, dan
          \texttt{ukur-ttfb.py} sengaja membebani, sehingga tidak boleh
          diarahkan ke sistem milik orang lain.
...
\begin{frame}{{\Large Langkah Kerja}}
  ...
  \begin{enumerate}
    ...
    \item Kirim satu permintaan \texttt{HEAD} ke \texttt{stylenew.css} situs
          kampus, lalu hitung masa \textit{fresh} tebakannya.
  \end{enumerate}}
  \vspace{4pt}
...
\begin{frame}{{\Large Langkah Kerja (Lanjutan)}}
  \vspace{6pt}
  \begin{enumerate}
    \setcounter{enumi}{4}
    \item Kirim satu permintaan bersyarat ke aset kampus yang sama.
    \item Unduh halaman aplikasi dan aset kampus tanpa header bersyarat, catat
          status dan jumlah bytenya.
    \item Muat ulang \texttt{http://localhost:8000/} dua kali di browser, catat
          kolom \emph{Status} dan \emph{Size}.
  \end{enumerate}
  \vspace{6pt}
  {\footnotesize Dua permintaan terakhir dibandingkan dengan permintaan
  bersyarat pada langkah sebelumnya: statusnya, dan jumlah byte yang
  diterima.}
\end{frame}
...
\begin{frame}{{\Large Tanda Versi Server Kampus}}
  ...
  Masa \textit{fresh} tebakan bergantung pada tanggal pemeriksaan.}
\end{frame}
...
\begin{frame}{{\Large Contoh Hasil Latihan 2}}
  ...
  lambat, karena dua kali penantian 50 milidetik ditambah \textit{query} yang tetap
  dijalankan.}
\end{frame}
...
\begin{frame}{{\Large Contoh Hasil Latihan 3}}
  ...
  dalam milidetik. Dengan satu permintaan, nilai \textit{stale} terbaca dalam 0,6
...
\begin{frame}{{\Large Lembar Isian Latihan 3}}
  ...
  besar daripada satu \textit{query} padahal \textit{query}-nya sama, mengapa \texttt{single\_flight}
  tetap satu \textit{query} berapa pun jumlah permintaannya, dan apa yang dikorbankan
  \texttt{stale\_while\_revalidate}?}
\end{frame}

\section{Latihan 4: Data Stale}

\begin{frame}{\hfill}
  \centering
  \Huge{\textbf{Latihan 4\\Data \textit{Stale} Setelah Perubahan}}
\end{frame}

\begin{frame}{{\Large Langkah Kerja}}
  \vspace{6pt}
  \begin{enumerate}
    \item Jalankan \texttt{invalidasi.py}, lalu catat jumlah pembacaan \textit{stale}
          dan lama \textit{stale}-nya untuk keempat cara.
...
\begin{frame}[fragile]{{\Large Mengukur Lama Data Stale}}
  \vspace{4pt}
\begin{lstlisting}[language=bash]
cd sources/chapter04
source ../.venv/bin/activate
...
\begin{frame}{{\Large Ringkasan}}
  \begin{itemize}
    \item \textit{Copy} yang masih \textit{fresh} dipakai tanpa jaringan. Jawaban 304 menghemat isi,
          tetapi tidak menghemat RTT.
    \item Tanpa kebijakan, browser menebak: aset kampus dianggap \textit{fresh} sekitar
          54 hari.
    ...
    \item Penghapusan yang kalah dalam \textit{race condition} bertahan 5,1
          detik. \textit{Single-flight} mengubah 50 \textit{query} serentak menjadi satu.
