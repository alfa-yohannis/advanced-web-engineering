...
\begin{frame}{{\Large Cache yang Macet}}
  ...
  \end{itemize}
  \vspace{6pt}
  {\footnotesize Aplikasi tetap menjawab karena galat Redis ditelan dan setiap
...
\begin{frame}{{\Large Dua Konsekuensi dan Satu Peringatan}}
  ...
  \begin{enumerate}
    \item Kegagalan \textit{cache} tidak boleh menjatuhkan aplikasi. Setiap
          galat Redis diteruskan menjadi pembacaan ke basis data.
...
\begin{frame}{{\Large Gagal Cepat dan Gagal Lambat}}
  ...
  \begin{itemize}
    ...
    \item Kegagalan lambat lebih berbahaya. Setiap permintaan yang menunggu
          menahan satu pekerja.
    \item Ketika semua pekerja menunggu, aplikasi berhenti melayani, termasuk
...
\begin{frame}{{\Large Cara Mengukurnya}}
  \vspace{10pt}
  \begin{itemize}
    \item \texttt{layanan-lambat.py}: layanan ongkos kirim tiruan yang biasanya
          menjawab dalam 20 milidetik, tetapi sebagian jawabannya ditahan 2
          detik.
    \item \texttt{resilience.py}: 10 pekerja bersama-sama mengirim 100
...
\begin{frame}{{\Large Membaca Tabel Kedua}}
  \vspace{10pt}
  \begin{itemize}
    \item Tanpa batas waktu, semuanya berhasil, tetapi 10 pekerja hanya mampu
...
\begin{frame}{{\Large Mengapa 10 Panggilan, Bukan 5}}
  ...
  \begin{itemize}
    ...
    \item Kesepuluh pekerja mulai bersamaan, sehingga panggilan pertama mereka
...
\end{lstlisting}
  ...
  paling lama satu detik. Batas 50 milidetik menjadi 4,26 sampai 4,90 detik.}
\end{frame}
...
\begin{frame}{{\Large Dua Pelengkap}}
  ...
  \begin{enumerate}
    \item Batas waktu mengecil ke arah \textit{downstream}. Bila seluruh permintaan diberi
          batas 1 detik, panggilan ongkos kirim di dalamnya diberi batas lebih
    ...
    \item Ketika \textit{circuit breaker} dalam keadaan \textit{open}, aplikasi tetap menjawab sesuatu: tarif
          terakhir yang tersimpan di \textit{cache}, atau pesan bahwa ongkos
...
\end{lstlisting}
  ...
  \vspace{2pt}
  {\footnotesize Basis data, Redis, dan layanan ongkos kirim dijalankan di
