...
\begin{frame}{{\Large Cara Kerja Tiap Jenis}}
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \hline
    Relasional & Data dipecah menjadi tabel normal, pemetaan relasi disusun saat \textit{query} lewat penggabungan \\
    \hline
...
\begin{frame}{{\Large Cocok untuk Apa, dan Apa Biayanya}}
  ...
  \begin{tabularx}{\textwidth}{lYY}
    ...
    \hline
    Graf & \textit{Social network}, rekomendasi, penelusuran hak akses berjenjang & \textit{Query} agregat biasa justru lebih lambat, dan penskalaannya rumit \\
    \hline
...
\begin{frame}{{\Large Dua Contoh Pemilihan}}
  ...
  \begin{enumerate}
    ...
    \item \textit{Dashboard} penjualan setahun terakhir dipindahkan ke basis
          data kolomnar. \textit{Query}-nya hanya menyentuh tiga kolom dari puluhan,
...
\begin{frame}{{\Large Tiga Tingkat Abstraksi}}
  ...
  \begin{itemize}
    ...
    \item \textit{Query builder} menyusun SQL dari objek tabel dan kolom,
          bukan dari teks. Bentuk \textit{query}-nya masih terbaca, dan nama kolom yang
          salah ketahuan sebelum \textit{query} dikirim.
    \item SQL langsung mengirim teks \textit{query} apa adanya.
  \end{itemize}
  \vspace{10pt}
...
\end{lstlisting}
  ...
  \texttt{pesanan.c.id}, bukan dari kelas. \textit{Query} dijalankan lewat koneksi
...
\end{lstlisting}
  ...
  \vspace{4pt}
  \begin{itemize}
    \item Teks \textit{query} dikirim apa adanya lewat \texttt{psycopg}.
...
\begin{frame}{{\Large Mana yang Dipilih?}}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \textit{Query builder} untuk \textit{query} yang bentuknya bergantung pada
            masukan pengguna, misalnya penyaringan dan pengurutan.
      \item SQL langsung untuk \textit{query} laporan dan \textit{query} yang perlu disetel.
    \end{itemize}
    \item Syaratnya, SQL yang benar-benar dikirim harus dapat dilihat:
          pencatatan \textit{query} saat pengembangan, dan jumlah \textit{query} per
          \textit{endpoint} saat pengujian.
  \end{itemize}
  \vspace{4pt}
  {\footnotesize \textit{Endpoint} adalah satu alamat yang dilayani server
  aplikasi, misalnya \texttt{GET /api/pesanan}. Jumlah \textit{query} per
  \textit{endpoint} berarti banyaknya \textit{query} SQL untuk melayani satu
...
\begin{frame}{{\Large Anomali Transaksi Bersamaan}}
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \hline
    \textit{Phantom read} & \textit{Query} yang sama dijalankan dua kali, dan jumlah barisnya bertambah. \\
    \hline
...
\begin{frame}{{\Large Contoh Anomali Saat Membaca}}
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \hline
    \textit{Phantom read} & A menghitung pesanan hari ini dan mendapat 120. B menambah satu pesanan, lalu melakukan \textit{commit}. A menjalankan \textit{query} yang sama dan mendapat 121. \\
    \hline
  \end{tabularx}}
...
\begin{frame}[fragile]{{\Large Query pada Cara Optimistis}}
  \vspace{2pt}
\begin{lstlisting}[style=PythonStyle]
...
\begin{frame}{{\Large Masalah N+1}}
  ...
  \vspace{8pt}
  {\footnotesize Hasilnya benar, tetapi cara memperolehnya boros: satu \textit{query}
  untuk mengambil daftar, lalu satu \textit{query} tambahan untuk tiap baris pada
  daftar tersebut.}
\end{frame}
...
\end{lstlisting}
  ...
  \vspace{2pt}
  {\footnotesize \textit{Query} kedua ada di dalam perulangan, sehingga dijalankan
  sekali untuk tiap pesanan: 25 pesanan berarti 1 + 25 = 26 \textit{query}.}
\end{frame}
...
\end{lstlisting}
  ...
  {\footnotesize
  \begin{itemize}
    \item \textit{Query} pertama mengambil item satu pesanan, sehingga harus dijalankan
          sekali untuk tiap pesanan. Inilah sumber N+1.
    \item \textit{Query} kedua menerima seluruh id pesanan sebagai larik lewat
          \texttt{ANY(\%s)}, sehingga cukup dijalankan sekali.
  \end{itemize}}
\end{frame}

\begin{frame}[fragile]{{\Large Dua Query, Berapa pun N-nya}}
  \vspace{2pt}
\begin{lstlisting}[style=PythonStyle]
...
\end{lstlisting}
  ...
  \vspace{2pt}
  {\footnotesize Pada 25 pesanan, 26 \textit{query} dalam 7,28 milidetik menjadi 2
  \textit{query} dalam 1,07 milidetik untuk 75 item yang sama persis. Satu \textit{query}
...
\begin{frame}{{\Large Angka Lokal Meremehkan Masalah}}
  ...
  \begin{itemize}
    ...
    \item 26 \textit{query} menanggung tambahan 26 milidetik, 2 \textit{query} hanya 2
  ...
  menghitung jumlah \textit{query} per \textit{endpoint} saat uji integrasi, lalu
...
\begin{frame}[fragile]{{\Large Query Acuan Q1 dan Q2}}
  \vspace{2pt}
...
\begin{frame}[fragile]{{\Large Query Acuan Q3 dan Q4}}
  \vspace{2pt}
...
\begin{frame}{{\Large Membuka Koneksi Itu Mahal}}
  ...
  \begin{itemize}
    ...
    \item Biayanya jauh lebih besar daripada menjalankan satu \textit{query}
          sederhana.
  \end{itemize}
  \vspace{10pt}
...
\begin{frame}{{\Large Hasil: 10 Worker, 50 Query Sepele}}
  \vspace{5pt}
  {\centering
  {\small
  ...
  lipat. Seluruhnya biaya membuka koneksi, karena \textit{query}-nya hanya
  \texttt{SELECT 1}.}
\end{frame}
...
\begin{frame}{{\Large Serverless: Pool per Instans}}
  ...
  \begin{itemize}
    ...
    \item Di \textit{serverless} tidak ada server yang menyala terus.
          Tiap \textit{request} yang datang bersamaan dilayani \textit{copy}
...
\section{Latihan 1: Baseline Query}

\begin{frame}{\hfill}
  \centering
  \Huge{\textbf{Latihan 1\\Menetapkan Baseline \textit{Query}}}
\end{frame}
...
\begin{frame}{{\Large Langkah Kerja}}
  ...
  \begin{enumerate}
    ...
    \item Untuk tiap \textit{query}, catat baris yang dikembalikan, \textit{node}
...
\begin{frame}{{\Large Lembar Isian Latihan 1}}
  ...
  \vspace{8pt}
  {\footnotesize Pertanyaan: \textit{query} mana yang membuang baris terbanyak
  dibanding baris yang dikembalikannya, \textit{query} mana yang punya \textit{node} Sort dan
  mengapa, serta mengapa p95 selalu lebih besar daripada p50 padahal \textit{query}
...
\begin{frame}{{\Large Langkah Kerja}}
  \vspace{5pt}
  \begin{enumerate}
    \item Tulis dugaan untuk tiap \textit{query}: apakah indeks akan terpakai, dan
...
\begin{frame}{{\Large Pertanyaan Analisis Latihan 3}}
  ...
  \begin{itemize}
    \item Waktu produksi dihitung dengan menambahkan 1 milidetik untuk tiap
          \textit{query}, sesuai RTT antara aplikasi dan basis data.
    \item Berapa kali lipat selisihnya pada pengukuran lokal, dan berapa kali
          lipat setelah RTT diperhitungkan?
    \item Mengapa jumlah \textit{query} cara kedua tetap dua, berapa pun jumlah
          pesanannya?
  \end{itemize}
\end{frame}
